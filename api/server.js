...
app.post('/api/admin/businesses', adminAuth, upload.single('image'), async (req, res) => {
  try {
    const { name, category_id, hood_id, website_link, phone_number, location_link, hook_text, price_indicator, business_type, address } = req.body;
    ...
    const { data, error } = await supabase.from('businesses').insert({
      ...
      address: address || null, rating_avg: 0, rating_count: 0,
    }).select().single();
    if (error) throw error;
